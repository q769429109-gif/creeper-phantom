...
import com.phantomblast.config.PhantomBlastConfig;
 ...
 *
 * <h2>只对苦力怕幻翼生效</h2>
 * <p>判定条件是 {@code source.getEntity() instanceof PhantomBlastEntity} ——
 * 注意是 {@link PhantomBlastEntity} 而不是 {@code Phantom}。
 ...
 * 伤害源就会让本方法再次命中"source.getEntity() instanceof PhantomBlastEntity"，
 ...
 */
@EventBusSubscriber(modid = PhantomBlast.MODID, bus = EventBusSubscriber.Bus.GAME)
public final class SwoopExplosionHandler {

    private static final Logger LOGGER = LogUtils.getLogger();

    /** 上一次引爆的游戏刻，写在实体自身的持久化数据里，随实体一起存盘/销毁，不用额外维护 Map。 */
    private static final String NBT_LAST_BLAST = "phantomblast:last_blast";
    ...
    public static void onLivingDamagePost(LivingDamageEvent.Post event) {
        // 1. 总开关
        if (!PhantomBlastConfig.ENABLED.get()) {
            return;
        }
        // 2. 重入保护
        if (detonating) {
        ...
        if (!(source.getEntity() instanceof PhantomBlastEntity blast)) {
            return;
        }

        // 7. 同一只生物的冷却
        long now = level.getGameTime();
        int cooldown = PhantomBlastConfig.COOLDOWN_TICKS.get();
     ...
     */
    private static void detonate(Level level, PhantomBlastEntity blast, Player victim) {
        double x = victim.getX();
        double y = victim.getY() + victim.getBbHeight() * 0.5D;
        double z = victim.getZ();

        PhantomBlastExplosionCalculator calculator = new PhantomBlastExplosionCalculator(
                PhantomBlastConfig.DESTROY_BLOCKS.get(),
                PhantomBlastConfig.PHANTOM_IMMUNE.get(),
                blast,
                victim,
                PhantomBlastConfig.DAMAGE_MULTIPLIER.get(),
                PhantomBlastConfig.HIT_PLAYER_DAMAGE_MULTIPLIER.get());

        // 破坏方块 = MOB：与苦力怕完全一致，受 mobGriefing 游戏规则约束
        // 不破坏方块 = NONE：保留对实体的伤害与击退，但地形无损
        Level.ExplosionInteraction interaction = PhantomBlastConfig.DESTROY_BLOCKS.get()
        ...
        try {
            level.explode(
                    ...
                    /* x, y, z          */ x, y, z,
                    /* radius           */ PhantomBlastConfig.EXPLOSION_POWER.get().floatValue(),
                    /* fire             */ PhantomBlastConfig.SET_FIRE.get(),
                    /* interaction      */ interaction);
        } finally {
            detonating = false;
        }

        if (PhantomBlastConfig.DEBUG_LOG.get()) {
            LOGGER.info("[PhantomBlast] 苦力怕幻翼({}) 在 ({}, {}, {}) 引爆，命中玩家 {}，威力 {}",
                    blast.getUUID(),
                    String.format("%.1f", x), String.format("%.1f", y), String.format("%.1f", z),
                    victim.getName().getString(),
                    PhantomBlastConfig.EXPLOSION_POWER.get());
        }
    }
