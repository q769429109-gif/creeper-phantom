 ...
 * <ol>
 *   <li>是个<E>b</b>不同的 {@code EntityType}（注册名 {@code phantomblast:creeperphantom}），
 *       所以它和原版幻翼可以同时存在于同一个世界、互不干扰。</li>
 *   <li>俯冲撕咬命中玩家时会引爆苦力怕级爆炸（逻辑在
 *       {@code com.phantomblast.explosion.SwoopExplosionHandler}）。</li>
 *   <li>用了自己的模型与贴图（客户端 {@code com.phantomblast.client} 包）。</li>
 * </ol>
 *
 * <p>除此之外——属性、生命、伤害、掉落、音效、粒子、被阳光点燃、和平模式消失、
 * 体型随机缩放、怕猫——<b>全部与幻翼一致</b>，因为它们都是继承来的。</p>
 */
public class PhantomBlastEntity extends Phantom {
     ...
     * {@code EntityType<? extends PhantomBlastEntity>} —— 因为 {@code super(...)}
     * 只接受前者，而 {@code EntityType<PhantomBlastEntity>} 同时满足两者，
     * 方法引用依然可以被 {@code EntityFactory<PhantomBlastEntity>} 接受。</p>
     */
    public PhantomBlastEntity(EntityType<? extends Phantom> type, Level level) {
        super(type, level);
    }
     ...
     * {@code phantomblast:entities/blast_phantom} —— 那个文件不存在，掉落就会是空的。
     ...
     * {@code data/phantomblast/loot_table/entities/blast_phantom.json} 并把这里删掉。</p>
     */
    @Override
