package com.phantomblast.entity;
 ...
 * </pre>
 * <p>下面每一项都对齐了，唯一区别是工厂方法指向 {@code PhantomBlastEntity::new}。</p>
 *
 * <p><b>注意这里没有调 {@code .noSummon()}</b>——原版幻翼也没有。
 * 所以 {@code /summon phantomblast:creeperphantom ~ ~10 ~} 可以直接用，
...
public final class ModEntities {
    ...
    /** 实体注册名（不含命名空间）。 */
    public static final String BLAST_PHANTOM_NAME = "creeperphantom";

    /** 本模组的实体类型注册器，由主类挂到 mod 事件总线上。 */
    public static final DeferredRegister<EntityType<?>> ENTITY_TYPES =
            DeferredRegister.create(Registries.ENTITY_TYPE, PhantomBlast.MODID);

    /**
     * 苦力怕幻翼。
     *
     * <p>{@code EntityType.Builder.<PhantomBlastEntity>of(...)} 这里显式写了类型见证：
     * 方法引用 {@code PhantomBlastEntity::new} 的形参是
     * {@code EntityType<? extends Phantom>}，与目标函数式接口
     * {@code EntityFactory<PhantomBlastEntity>} 要求的 {@code EntityType<PhantomBlastEntity>}
     * 并不完全相同（泛型不协变），靠逆变规则是可以推断的，
     * 但显式写出更省得编译器抱怨。</p>
     */
    public static final DeferredHolder<EntityType<?>, EntityType<PhantomBlastEntity>> BLAST_PHANTOM =
            ENTITY_TYPES.register(BLAST_PHANTOM_NAME, () -> EntityType.Builder
                    .<PhantomBlastEntity>of(PhantomBlastEntity::new, MobCategory.MONSTER)
                    .sized(0.9F, 0.5F)
                    .eyeHeight(0.175F)
                    .passengerAttachments(0.3375F)
                    .ridingOffset(-0.125F)
                    .clientTrackingRange(8)
                    .build(ResourceLocation.fromNamespaceAndPath(PhantomBlast.MODID, BLAST_PHANTOM_NAME)
                            .toString()));
    ...
    public static final DeferredHolder<EntityType<?>, EntityType<WaterCreeperEntity>> WATER_CREEPER =
            ENTITY_TYPES.register(WATER_CREEPER_NAME, () -> EntityType.Builder
                    ...
                    .build(ResourceLocation.fromNamespaceAndPath(PhantomBlast.MODID, WATER_CREEPER_NAME)
                            .toString()));
