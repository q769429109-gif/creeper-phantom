 ...
 */
@EventBusSubscriber(modid = PhantomBlast.MODID, bus = EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
public final class PhantomBlastClient {

    /** 注册苦力怕幻翼自己的模型图层。 */
    @SubscribeEvent
    public static void onRegisterLayerDefinitions(EntityRenderersEvent.RegisterLayerDefinitions event) {
        event.registerLayerDefinition(PhantomBlastModel.LAYER, PhantomBlastModel::createBodyLayer);
    }

    /** 给两只生物挂上各自的渲染器。 */
    @SubscribeEvent
    public static void onRegisterRenderers(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.BLAST_PHANTOM.get(), PhantomBlastRenderer::new);
