package com.phantomblast;

import com.phantomblast.config.PhantomBlastConfig;
import com.phantomblast.entity.ModEntities;
import com.phantomblast.entity.WaterCreeperEntity;
import com.phantomblast.item.ModItems;
...
/**
 * 幻翼爆破 Phantom Blast —— 模组主入口。
 *
 * <h2>这个模组做什么</h2>
 * <p>新增一个独立生物 <b>苦力怕幻翼（Creeper Phantom，{@code phantomblast:creeperphantom}）</b>：
 ...
 *   <li>{@code PhantomBlastSpawnHook} —— 自然生成器注入（游戏事件总线）；</li>
 *   <li>{@code SwoopExplosionHandler} —— 俯冲引爆逻辑（游戏事件总线）；</li>
 *   <li>{@code PhantomBlastClient} —— 模型图层与渲染器（mod 事件总线，仅客户端）。</li>
 * </ul>
 */
@Mod(PhantomBlast.MODID)
public class PhantomBlast {

    public static final String MODID = "phantomblast";

    public PhantomBlast(IEventBus modEventBus, ModContainer modContainer) {
        ...
        modEventBus.addListener(PhantomBlast::onEntityAttributeCreation);

        // 把刷怪蛋放进创造模式的「刷怪蛋」标签页。
        modEventBus.addListener(PhantomBlast::onBuildCreativeTabContents);

        // 水下苦力怕的生成位置规则（水中 + 阴暗环境）。
        modEventBus.addListener(PhantomBlast::onRegisterSpawnPlacements);
        ...
        // 用 COMMON 而非 SERVER：配置文件落在 config/phantomblast-common.toml，
        // 单机与专用服务器都是同一个固定路径，方便用户直接改。
        ConfigTracker.INSTANCE.registerConfig(
                ModConfig.Type.COMMON,
                PhantomBlastConfig.SPEC,
                modContainer);
    }
    ...
    private static void onEntityAttributeCreation(EntityAttributeCreationEvent event) {
        event.put(ModEntities.BLAST_PHANTOM.get(), Monster.createMonsterAttributes().build());
    ...
    private static void onRegisterSpawnPlacements(RegisterSpawnPlacementsEvent event) {
        event.register(
                ...
                PhantomBlast::checkWaterCreeperSpawnRules,
