...
- **提高苦力怕幻翼在怪物池里的权重：20 → 60**。
  参照原版平原怪物池：僵尸 95、骷髅/苦力怕/蜘蛛 各 100、末影人 10 ——
  20 只有约 4% 的中签率，在已经很挤的世界里几乎看不见；60 约 10%，入夜就能稳定遇到。
  ⚠️ **仍然受怪物全局上限约束**：`MobCategory.MONSTER` 每 289 个可刷区块最多 70 只。

### 排查记录（主人反馈「晚上怪很多，却没有苦力怕幻翼」）

- 1.11.0 已正确部署（jar 内数据包、新谓词都在，旧存根类没进产物），机制本身没问题。
- 实测存档：玩家周围 ±8 区块内有 **僵尸 46 / 骷髅 16 / 蜘蛛 16 / 苦力怕 15 / 末影人 1 = 94 只**，
  而 MONSTER 上限是 **70** —— **上限已满，所有怪物（包括我们的）都停止刷新**。
  主人看到的"到处都是怪"是积压的存量，不是新刷出来的。
- 这也是本次改机制的一个副作用，需要知道：幻翼**原本不走刷怪上限**
  （CustomSpawner 无上限），改成独立怪物后就和僵尸一样受 70 上限与权重竞争约束了。

### 怎么验证

```
/kill @e[type=!minecraft:player]      ← 清场，腾出上限（注意：掉落物也会被清）
/time set night                        ← 确保是夜晚
```
等约 1 分钟，苦力怕幻翼就会随怪物刷新出现。
